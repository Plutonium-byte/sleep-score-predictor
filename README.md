...
This project aims to predict an individual’s **Quality of Sleep** based on their health and lifestyle habits using multiple machine learning models.
...
The dataset contains information about individuals’ **sleep habits, physical activity, stress levels, BMI category, and other lifestyle factors**.
...
```
├── train.py          # Trains all models and saves the best one as model.bin
├── predict.py        # Flask-based API (containerized with Docker) that serves predictions
├── serve.py          # Sends requests to the Flask API (running in Docker) and retrieves predictions
...
├── dataset.csv       # Sleep Health and Lifestyle dataset (from Kaggle)
├── uv.lock           # Locked dependency versions for reproducibility 
├── Dockerfile        # Defines the container for running predict.py
├── model.bin         # Saved trained model used by predict.py
├── .python-version   # Specifies Python version used in the project       
...
### 🔹 Normal Train (Partial Training Dataset (60%) )
...
### 🔹 Full Train (Complete Dataset (80%) )
